...
#Author : finance
#MailTo : financezone@foxmail.com
#QQ     : 648719819
#Blog   : http://www.cnblogs.com/finance
#Create : 2013-08-09
#Version: 1.0
...
数据库文件，而是直接打开该数据库文件。
    ...
    conn = sqlite3.connect('c:\\test\\finance.db')
...
def create_table_test():
   ...
   #                     )'''
    create_table_sql = '''CREATE TABLE `Stocks` (
                          ...
                          `concept` varchar(1) DEFAULT NULL,
                           PRIMARY KEY (`code`)
                        )'''
    conn = get_conn(DB_FILE_PATH)
    create_table(conn, create_table_sql)

def save_test(table,data):
    '''保存数据测试...'''
    print('保存数据测试...')
    save_sql = '''INSERT INTO '''+table+''' values (?, ?, ?, ?, ?, ?)'''
   # data = [(1, 'finance', '男', 20, '广东省广州市', '13423****62'),
...
def delete_test(table):
    '''删除数据...'''
    print('删除数据...')
    delete_sql = 'DELETE FROM '+table+' WHERE  code = ? '
    conn = get_conn(DB_FILE_PATH)
    data=[('1')]
